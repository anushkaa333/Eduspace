...
if(isset($_POST['apply']))
{
  ...
  $sid=$_SESSION['sid'];
  $cid=$_POST['cid'];
  $q= 'insert into apply (sid,cid,status,add_date) values ('.$sid.','.$cid.',"applied",current_date());';
...
if(isset($_POST['apply']))
{
  ...
  $sid=$_SESSION['sid'];
 $tid=$_POST['tid'];
  $q= 'insert into contact (sid,tid) values ('.$sid.','.$tid.');';
   $result = mysqli_query($con,$q);
   if($result==true)
{
  echo 'sucess';
      ...
      <div class="container">
        ...
        <div id="collapsable-nav" class="collapse navbar-collapse">
          ...
          <li class="nav-item home custom-nav-item ">
              <a class="nav-link active" href="#" style="margin-right: 10px;">Home<span class="sr-only">(current)</span></a>
            </li>
...
<form action="stud_home.php" method="post">
  <input type="text" name="valuetosearch" placeholder="What do yo want to learn?" style="margin-left: 400px; text-align: center; ">
  <button type="submit" name="search" value="search" style="width: 60px; height:42px; text-align: center; background: #225470; border:none;"><i 
...
if(isset($_POST['search']))
{
  ...
  $q1 = "select * from course where cname = '$valuetosearch' ";
  ...
  <div class="des">
    <p>'.$row['description'].'</p>
      <form action ="stud_home.php" method= "post" ><input type="hidden" name="cid" value='.$row['cid'].'><input type="hidden" name="tid" value='.$row['tid'].'><button name="apply" class="btn">apply</form>
  </div>
</div></div></center><br>';
  }
}
...
<h1 class="text-center" style="font-size:50px; color: black; margin-top: -7px;">All Courses</h1>
<header style=" background:url('images/—Pngtree—admissions training course flyer_1052395.jpg') no-repeat center center/cover; padding:52px;">
<?php
